...
final class SequenceAsyncMapChunkSizeTests: XCTestCase {
    func testAsyncMapWithChunkSize() async throws {
        ...
        let result = await numbers.asyncMap(chunkSize: 2) { number in
            await Task.yield()
            return number * 2
        }
    ...
    func testAsyncMapWithDifferentChunkSizes() async throws {
        ...
        // チャンクサイズが要素数より大きい場合
        let result1 = await numbers.asyncMap(chunkSize: 5) { number in
            await Task.yield()
            return number * 2
        }
        ...
        let result2 = await numbers.asyncMap(chunkSize: 1) { number in
            await Task.yield()
            return number * 2
        }
